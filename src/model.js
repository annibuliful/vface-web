import { Live2DCubismFramework } from "../CubismSdkForWeb-4-r.3/Framework/src/live2dcubismframework.ts";
import { Live2DCubismFramework as cubismusermodel } from "../CubismSdkForWeb-4-r.3/Framework/src/model/cubismusermodel.ts";
import { Live2DCubismFramework as cubismmodelsettingjson } from "../CubismSdkForWeb-4-r.3/Framework/src/cubismmodelsettingjson.ts";
import { Live2DCubismFramework as cubismeyeblink } from "../CubismSdkForWeb-4-r.3/Framework/src/effect/cubismeyeblink.ts";
import { Live2DCubismFramework as cubismdefaultparameterid } from "../CubismSdkForWeb-4-r.3/Framework/src/cubismdefaultparameterid.ts";
import { Live2DCubismFramework as cubismbreath } from "../CubismSdkForWeb-4-r.3/Framework/src/effect/cubismbreath.ts";
import { Live2DCubismFramework as csmvector } from "../CubismSdkForWeb-4-r.3/Framework/src/type/csmvector.ts";
import { Live2DCubismFramework as cubismmatrix44 } from "../CubismSdkForWeb-4-r.3/Framework/src/math/cubismmatrix44.ts";
...
export default class extends CubismUserModel {
  ...
  async initSettings() {
    const settings = await (
      await fetch(`${Config.MODEL_DIR}/${Config.MODEL_SETTINGS}`)
    ).arrayBuffer();
  ...
  async initModel() {
    const model = await (
      await fetch(`${Config.MODEL_DIR}/${this.settings.getModelFileName()}`)
    ).arrayBuffer();
  ...
  initRenderer() {
    ...
    renderer.setRenderState(this.gl.getParameter(this.gl.FRAMEBUFFER_BINDING), [
      0,
      0,
      this.canvas.width,
      this.canvas.height,
  ...
  async initPhysics() {
    const physics = await (
      await fetch(`${Config.MODEL_DIR}/${this.settings.getPhysicsFileName()}`)
    ).arrayBuffer();
  ...
  initBreath() {
    ...
    breathParameters.pushBack(
      new BreathParameterData(this._idParamBreath, 0, 0.2, 6, 0.2)
    );
    breathParameters.pushBack(
      new BreathParameterData(this._idParamBodyAngleY, 0, 0.1, 6, 0.1)
    );
